...
namespace GPOOHLT
{
    ...
    {
        List<Doctor> m_objList = new List<Doctor>();
        ...
        private void ReadAndSetData(DataTable objDT)
        {
            ...
            foreach (DataRow row in objDT.Rows)
            {
                ...
                m_objList.Add(new Doctor()
                {
                    ...
                    // ---------------------------------------------------------
                    Referral = Convert.ToString(row["Referral"]),
                    Medication = Convert.ToString(row["Medication"])
                    // ---------------------------------------------------------
                });
            }
        }
        ...
        private string SaveDoctorData()
        {
            userControlBrowse1.SaveInXML();

            string result = string.Empty;
            int count = 1;
            foreach (Doctor obj in m_objList)
            {
                ...
                Common.SaveDoctorConsultation(obj, ref count);
